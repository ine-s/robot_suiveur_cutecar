...
- `SW(7 downto 0)` : entrée 8 bits lue via le PIO d'entrée pour les switches (`sw_export`).
- `LED(7 downto 0)` : sortie 8 bits pilotée via le PIO de sortie pour les leds(`led_export`).
...
## Rôle du programme C
Le fichier `app_software/niosII_light_v1.c` réalise une boucle de test de la chaîne
NIOS -> bus Avalon -> PIO :
